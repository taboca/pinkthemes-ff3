...
def main():
    ...
    tableUp = { "normal": [ color_r, color_g, color_b ], "1": [ 0,0,0], "2": [0,0,0], "3": [0,0,0], "4":[0,0,0] , "5":[0,0,0], "6":[0,0,0], "7":[0,0,0] } 
 
    f = open ( "sedcolorlow.sh" , "w");
	...
	if cur != "normal":
	 	numCur = int ( cur ) 
		if numCur: 
			rCur = color_r - rDown*numCur; 
			gCur = color_g - gDown*numCur; 
			bCur = color_b - bDown*numCur; 
	...
	try: 
		...
		f.write( "sed -e \"s/colorlow" + cur + "/rgb(" + str( tableUp[cur][0]) + "," + str (tableUp[cur][1]) + "," + str(tableUp[cur][2]) +")/\" " + infile + "\n"  )
		f.write( cpline + "\n" );
